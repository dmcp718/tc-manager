    ...
    try {
      ...
      await FFmpegTranscoder.transcodeToHLSProgressive(
        ...
        cacheKey,
        {
          ...
          VIDEO_MAXRATE: this.VIDEO_MAXRATE,
          VIDEO_BUFSIZE: this.VIDEO_BUFSIZE,
          ...
          HLS_SEGMENT_TIME: this.HLS_SEGMENT_TIME,
...
class FFmpegTranscoder {
  ...
  static async transcodeToHLSProgressive(inputPath, outputDir, videoId, config, onProgress, onSegmentReady) {
    return new Promise(async (resolve, reject) => {
      ...
      args.push(
        ...
        '-filter:v:0', `scale=w=${config.VIDEO_WIDTH}:h=${config.VIDEO_HEIGHT}:force_original_aspect_ratio=decrease,pad=${config.VIDEO_WIDTH}:${config.VIDEO_HEIGHT}:(ow-iw)/2:(oh-ih)/2`,
        '-maxrate:v:0', config.VIDEO_MAXRATE,
        '-bufsize:v:0', config.VIDEO_BUFSIZE,
        '-b:v:0', config.VIDEO_BITRATE
      );
      ...
      if (hasAudio) {
        args.push(
          ...
          '-ac', config.AUDIO_CHANNELS.toString(),
      ...
      args.push(
        ...
        '-hls_time', config.HLS_SEGMENT_TIME.toString(),
